...
namespace Juan_Back_End_Final.Areas.Manage.Controllers
{
    ...
    public class SliderController : Controller
    {
        ...
        public async Task<IActionResult> Create(Slider slider, bool? status, int page = 1)
        {
            ...
            slider.SupTitle = slider.SupTitle.Trim();
            slider.Title = slider.Title.Trim();
            slider.Description = slider.Description.Trim();

            Regex regex = new Regex(@"\s{2,}");
            if (regex.IsMatch(slider.SupTitle) && regex.IsMatch(slider.Title) && regex.IsMatch(slider.Description))
            {
                ModelState.AddModelError("SupTitle", "Should not be Space");
        ...
        public async Task<IActionResult> Update(int? id, Slider slider, bool? status, int page = 1)
        {
            ...
            dbSlider.SupTitle = slider.SupTitle;
